...
public class View extends JFrame implements ActionListener, PeopleChangedListener {
	...
	public void onPeopleChanged() {
		/*
		 * Some interpretations of MVC would force the view to be updated only via the
		 * controller, which would contact the database, get the data and tell the view
		 * to update itself (by calling a view method directly). Others, as here, have
		 * the view listening to the model (but never telling it what to do).
		 */
		listModel.clear();
